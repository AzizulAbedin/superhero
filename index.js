// Comment: Declaring and initializing variables and selecting DOM elements
const allTabsBody = document.querySelectorAll('.tab-body-single');
const allTabsHead = document.querySelectorAll('.tab-head-single');
const searchForm = document.querySelector('.app-header-search');
let searchList = document.getElementById('search-list');

let activeTab = 1; // Comment: Setting the initial active tab to 1
let allData; // Comment: Declaring a variable for storing all data

// Comment: Function for initializing the code
const init = () => {
    showActiveTabBody();
    showActiveTabHead();
}

// Comment: Function to show the active tab head
const showActiveTabHead = () => allTabsHead[activeTab - 1].classList.add('active-tab');

// Comment: Function to show the active tab body
const showActiveTabBody = () => {
    hideAllTabBody();
    allTabsBody[activeTab - 1].classList.add('show-tab');
}

// Comment: Function to hide all tab bodies
const hideAllTabBody = () => allTabsBody.forEach(singleTabBody => singleTabBody.classList.remove('show-tab'));

// Comment: Function to hide all tab heads
